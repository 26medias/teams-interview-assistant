 ...
 *
 * We track each entry by its avatar ID, and emit "utterance-end" when
 * an entry's text stops changing (finalized) or when a new entry appears.
 */
export async function startCaptionScraping(
    ...
    const FINALIZE_MS = 2500; // Emit after text stops changing for 2.5s
    let stopped = false;
            ...
            for (const raw of currentEntries) {
                ...
                if (existing) {
                    ...
                    if (raw.text !== existing.text) {
                        ...
                        // Don't re-emit if already emitted (text can grow)
                        if (existing.emitted) {
                            existing.emitted = false; // Re-arm: text grew
                        }
                    }
                } else {
                    // New entry appeared — finalize all previous entries from the same
                    // or different speaker that haven't been emitted yet
